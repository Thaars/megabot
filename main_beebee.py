...
async def websocket_connect(access_token, ws_url):
    ...
    init_msg = None
    init_s = None
    # this is a good habit of recording certain sequence numbers to match when you get a response from the websocket api
    a_seq_n = 0
...
async def marketdata_run(management_pkg):
    ...
    ws = await websocket_connect(access_token, ws_url)
    if ws:
    ...
    while running:
        ...
        if md_hb_value.value >= 1.0:
            try:
                ...
                print(
                    "\nmarketdata_run -- WEBSOCKET -- sent heartbeat EXCEPTION -- [{0}] [{1}]".format(md_hb_value.value,
                                                                                                      str(e)))
