...
def start_emas(limit):
    # for limit in frange(103, 103, 0.5):
    for stop in frange(97.5, 98, 0.5):
        for ema_short in range(8, 10):
            for ema_long in range(12, 15):
                if ema_short < ema_long - 2 and 100 - stop <= limit - 100:
                    # for adx in range(17, 23):
                    adx = 17
                    test_single_symbol(SYMBOL, stop, limit, ema_short, ema_long, adx, False)
...
def test(df, pf, stop, limit, ema_short, ema_long, adx):
    df, pf = strategy.ma_crossing_adx(df, pf, ema_short, ema_long, stop, limit, adx)
...
def test_single_symbol(item, stop, limit, ema_short, ema_long, adx, chart=False):
    ...
    performance = test(df, pf, stop, limit, ema_short, ema_long, adx)
